    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
        ...
        <form action="{{ route('admin.users.update', $user) }}" method="POST" novalidate>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6">
                ...
                <div>
                    ...
                    <!-- <select name="status" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500">
                        @foreach(config('identity.statuses', ['active', 'inactive', 'suspended', 'pending']) as $status)
                            <option value="{{ $status }}" {{ old('status', $user->status->label()) == $status ? 'selected' : '' }}>
                                {{ ucfirst($status) }}
                            </option>
                        @endforeach
                    </select> -->
                    <select name="status" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500">
                        @foreach(\Modules\Identity\Enums\UserStatus::cases() as $status)
                            <option 
                                value="{{ $status->value }}"
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1.5">New Password (Leave blank to keep current)</label>
